...
如果你是一名新手，在使用 Laravel Fortify 之前不妨尝试使用 [Laravel Breeze](/docs/laravel/9.x/starter-kits) 应用入门套件。Laravel Breeze 为你的应用提供身份认证支架，其中包括使用 [Tailwind CSS](https://tailwindcss.com)。与 Fortify 不同，Breeze 将其路由和控制器直接发布到你的应用程序中。这使你可以学习并熟悉 Laravel 的身份认证功能，然后再允许 Laravel Fortify 为你实现这些功能。
...
该命令会将 Fortify 的行为类发布到你的 `app/Actions` 目录，如果该目录不存在，则会创建该目录。此外，还将发布 Fortify 的配置和迁移文件。
...
Laravel Fortify 通过可调用类的管道对登录请求进行身份验证。如果你愿意，你可以定义一个自定义的类管道，登录请求应该通过管道传输。每个类都应该有一个 `__invoke` 方法，该方法接收传入 `Illuminate\Http\Request` 实例的方法，并且像 [中间件](/docs/laravel/9.x/middleware) 一样，调用一个 `$next` 变量，以便将请求传递给管道中的下一个类。
...
如果请求不成功，用户将被重定向回请求密码重置链接屏幕，验证错误将通过共享的 `$errors` [Blade 模板变量](/docs/laravel/9.x) 提供给你/validation#quick-displaying-the-validation-errors）。或者，在 XHR 请求的情况下，验证错误将返回 422 HTTP 响应。
...
你的“验证电子邮件”模板应包含一条信息性消息，指示用户单击发送到其电子邮件地址的电子邮件验证链接。
