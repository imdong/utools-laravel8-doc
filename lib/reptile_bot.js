...
(function (module) {
    ...
    // const pygmentize = require('pygmentize')
    ...
    class Bot {
        ...
        run() {
            ...
            // 初始化代码高亮功能
            Highlight.init((err) => {
                // console.log('Highlight.init', err, Highlight);
            }, ['php'], {
                classPrefix: 'token '
            });
            ...
            marked.setOptions({
                highlight: function (code, lang, callback) {
                    ...
                    dom('span').map((index, item) => {
                        $(item).addClass('token');
                    })

                    return dom('div').html();

                    pygmentize({ lang: lang || 'php', format: 'html' }, code, function (err, result) {
        ...
        loadMenu() {
            ...
            axios.get(this.options.source_url + '/8.x', this.axios_options).then((response) => {
        ...
        getPageList($) {
            ...
            $('ol.sorted_table ol.chapter-container li.item').each((index, element) => {
                ...
                this.page_list.push({
                    ...
                    html_path: this.options.html_path + '/' + page_name + '.html'
                });
            });
        ...
        buildIndexs() {
            ...
            fs.writeFile('./plugin/indexes.json', JSON.stringify(indexs), () => {
                console.log('Exit');
            });
        }
        ...
        buildDocs(md_body, item) {
            // 转换 url [Laravel 支持的](/docs/laravel/8.x/database#introduction)
            md_body = md_body.replace(/\/docs\/laravel\/8\.x\/([^#\)]+)/g, (full, match) => {
                return match + '.html';
            });
            ...
            // 渲染代码
            // html = Highlight.highlight(html, null, true);

            // 替换模板
            html = this.options.doc_tpl.replace('{MarkdownBody}', html);


            fs.writeFile(item.html_path, html, (err, fd) => { });
        }
    }
